...
//Temperature Controlling LED2
        ...
        {
            if(temp >= temproom * 1.02) { P1OUT |=  BIT5; __delay_cycles(200); }//On
            if (temp <= temproom) { P1OUT &= ~BIT5; __delay_cycles(200); }//Off
        }
...
//The following code uses a dead zone of no action between 0.7-0.9 of the value touch
        ...
        if(touch > touchroom * 0.96 && touch < touchroom)
        ...
        else
          {
            ...
            if(touch <= touchroom * 0.96 && flag == 1) {P2OUT ^=  BIT0; __delay_cycles(200); flag=0;}  //LED toggle
          }
...
void getanalogvalues()
{
 i = 0; temp = 0; light = 0; touch =0;        // set all analog values to zero
    for(i=1; i<=5 ; i++)                      // read all three analog values 5 times each and average
  {
    ...
    while (ADC10CTL1 & BUSY);                 //Wait while ADC is busy
    light += ADCReading[1];                   // sum  all 5 reading for the three variables
    touch += ADCReading[0];
    temp += ADCReading[2];
  }
