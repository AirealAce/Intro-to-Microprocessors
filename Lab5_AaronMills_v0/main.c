...
P1DIR = 0xff; // port 1 all output Click to BUY NOW!
...
// SW-321 = 011: right and left digits both hold previously set values (preset value)
if (((switches & BIT5) != BIT5) && ((switches & BIT4) == BIT4) && ((switches & BIT3) == BIT3))
...
// SW-321 = 100: Counter cycles up from preset value to 90, and repeat
if (((switches & BIT5) == BIT5) && ((switches & BIT4) != BIT4)&& ((switches & BIT3) != BIT3))
// *** modify this section according to the lab manual requirement
{ //Check if counter is 90 or above
if (leftdigit >= 9 & rightdigit == 0) {leftdigit=pleftdigit; rightdigit=prightdigit;}
//otherwise count up until counter is 90
else {//Countup Algorithim
if (rightdigit==9) {++leftdigit; rightdigit=0;}
else {++rightdigit;};
}
}
// SW-321 = 111: Counter cycles down from preset value to 10, and repeats
if (((switches & BIT5) == BIT5) && ((switches & BIT4) == BIT4)&& ((switches & BIT3) == BIT3))
// *** modify this section according to the lab manual requirement
{
...
else {//Countdown Algorithim

if (rightdigit==0) {--leftdigit; rightdigit=9;}
else {--rightdigit;};
};
}
